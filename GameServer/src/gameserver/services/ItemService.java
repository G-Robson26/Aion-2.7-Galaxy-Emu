   ...
   /**
    ...
    public static void addRepurchaseItem(Player player, Item item)
	{
		...
		for(Item existingItem : existingItems)
		{
			...
			sendStorageUpdatePacket(player, 0, existingItem);
		...
		while(!destinationStorage.isFull() && count > 0)
		{
			...
			{
				count -= maxStackCount;
				Item newitem = newItem(item.getItemTemplate().getTemplateId(), maxStackCount, item.getItemCreator());
				newitem = destinationStorage.putToBag(newitem);
				sendStorageUpdatePacket(player, 0, newitem);
			}
			else
			{
				item.setItemCount(count);
