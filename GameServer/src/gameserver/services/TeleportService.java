...
public class TeleportService
{
    ...
    public static void regularTeleport(TeleporterTemplate template, int locId, Player player)
    {
        ...
        PacketSendUtility.sendPacket(player, new SM_TELEPORT_LOC(locationTemplate.getMapId(), locationTemplate.getX(),
            locationTemplate.getY(), locationTemplate.getZ(), 4));
		...
		final float y, final float z, final byte heading, final int delay)
	{
		...
		if(DuelService.getInstance().isDueling(player.getObjectId().intValue()))
		...
		PacketSendUtility.sendPacket(player, new SM_ITEM_USAGE_ANIMATION(player.getObjectId().intValue(), 0, 0, delay, 0, 0));
		ThreadPoolManager.getInstance().schedule(new Runnable(){
			...
			public void run()
			{
				...
				PacketSendUtility.sendPacket(player, new SM_ITEM_USAGE_ANIMATION(player.getObjectId().intValue(), 0, 0, 0, 1, 0));
				changePosition(player, worldId, instanceId, x, y, z, heading);
			}
		}, delay);
