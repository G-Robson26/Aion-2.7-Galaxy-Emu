...
public class EnchantService {
    ...
    public static boolean enchantItem(Player player, Item parentItem, Item targetItem, Item supplementItem) {
        ...
        if (supplementItem != null) {
			...
			//Felicitous Socketing (Fabled) 100%
			if(supplementId == 166150003)
				addsuccessRate = EnchantsConfig.FELSOCFAB;
			//Felicitous Socketing (Eternal) 100%
			if(supplementId == 166150004)
				addsuccessRate = EnchantsConfig.FELSOCETE;
			//Blessed Manastone Socketing Supplements (Hero) 100% (lvl 60)
			if(supplementId == 166150015)
				addsuccessRate = EnchantsConfig.BLESOCHERO;
			//Blessed Manastone Socketing Supplements (Fabled) 100% (lvl 60)
			if(supplementId == 166150014)
				addsuccessRate = EnchantsConfig.BLESOCETE;	
    ...
    public static boolean socketManastone(Player player, Item parentItem, Item targetItem, Item supplementItem, int targetWeapon) {
        ...
        if (targetWeapon == 1) {
            if (stoneCount >= targetItem.getSockets(false))
                successRate = EnchantsConfig.MSPERCENT6;
        } else {
