...
public class PlayerController extends CreatureController<Player> {
    ...
    public void onEnterWorld() {
		...
		if ((getOwner().getWorldId() == 300300000) && (!getOwner().getInAcademy()) && (getOwner().getPlayerGroup() != null)) {
        PacketSendUtility.sendPacket(getOwner(), new SM_INSTANCE_SCORE(getOwner().getWorldId(), (int)(getOwner().getPlayerGroup().getInstanceStartTime() + 14400000L - System.currentTimeMillis()), getOwner().getPlayerGroup(), false));
        getOwner().setInAcademy(true);
        }
