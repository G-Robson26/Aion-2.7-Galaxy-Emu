...
public class PortalController extends NpcController {
	...
	public static void setInstanceCooldown(Player paramPlayer, int paramInt1, int paramInt2) {
        int i = DataManager.WORLD_MAPS_DATA.getTemplate(paramInt1).getInstanceMapId();
        if (paramPlayer.getInstanceCD(i) == null)  {
        ...
        if (paramPlayer.isInGroup())
        paramPlayer.addInstanceCD(i, localTimestamp, paramInt2, paramPlayer.getPlayerGroup().getGroupId());
        else
        paramPlayer.addInstanceCD(i, localTimestamp, paramInt2, 0);
       } 
	}
