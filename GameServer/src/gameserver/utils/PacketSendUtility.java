...
public class PacketSendUtility {
	...
	public static void broadcastPacketToGroup(Player player, AionServerPacket packet) {
        for(Player member : player.getPlayerGroup().getMembers()) {
            sendPacket(member, packet);
        }
    }
    
    public static void broadcastPacketToGroup(Player player, AionServerPacket packet, Player excludedPlayer) {
        for(Player member : player.getPlayerGroup().getMembers()) {
            if(!member.equals(excludedPlayer))
                sendPacket(member, packet);
        }
    }
}
