...
import gameserver.model.templates.item.TradeInTemplate;
...
public class ItemTemplate extends VisibleObjectTemplate {
	...
	@XmlElement(name = "trade_items", required = false)
    protected TradeInTemplate trade_items;
	
	@XmlAttribute(name = "inzone", required = false)
 	private inZone inzone;	
    ...
    public boolean isAllowedFor(PlayerClass playerClass, int level) {        
		...
		if(!related && !playerClass.isStartingClass())
		{
			related = restricts[PlayerClass.getStartingClassFor(playerClass).ordinal()] > 0 && restricts[playerClass.ordinal()] <= level;
		}
		return related;
    }
	...
	/**
     * @return the trade in template
     */
    public TradeInTemplate getTradeInTemplate() 
	{
        return trade_items;
    }
