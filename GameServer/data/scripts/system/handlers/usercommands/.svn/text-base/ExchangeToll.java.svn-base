...
public class ExchangeToll extends UserCommand {
    ...
    public void executeCommand(Player player, String params) {
        ...
        if (params.toLowerCase().indexOf("ap") != -1) {
        	...
        	//Exchange Amount into Toll (default is Kinah, but overridden if restriction is AP
        ...
        if (playerAccount != null) {
        	int currentToll = playerAccount.getToll();
        	playerAccount.SetToll(currentToll + tollToGain);
        }

    	PacketSendUtility.sendMessage(player, LanguageHandler.translate(CustomMessageId.COMMAND_EXCHANGETOLL_SUCCESS, tollToGain));
    }
