        ...
        if (qs.getStatus() == QuestStatus.START) {
            if (env.getTargetId() == 799553) {
                switch (env.getDialogId()) {
                    case 26:
                        if (var == 0) {
                            if (player.getInventory().getItemCountByItemId(182215002) != 0)
                    ...
                    case 1353:
                        if (var == 0) {
                            player.getInventory().removeFromBagByItemId(182215002, 1);
                            ItemService.addItems(player, Collections.singletonList(new QuestItems(182215024, 1)));
                        }
                        return false;
                    case 10000:
                        if (var == 0) {
			...
			}else if (env.getTargetId() == 799552) {
                switch (env.getDialogId()) {
                    case 26:
                        if (var == 1)
                            return sendQuestDialog(env, 2375);
                    case 1009:
                        defaultQuestRemoveItem(env, 182215024, 1);
                        return defaultCloseDialog(env, 1, 1, true, true);
                }
            }
        }
        return defaultQuestRewardDialog(env, 799552, 0);
    }
}