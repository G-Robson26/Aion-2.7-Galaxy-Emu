...
public class MySQL5PlayerPetsDAO extends PlayerPetsDAO
{
    ...
    public boolean savePet(Player player, ToyPet pet) {
        ...
        try {
            con = DatabaseFactory.getConnection();
            PreparedStatement stmt = con.prepareStatement("UPDATE player_pets SET feed_count = ?, love_count = ?, exp = ?, feed_state = ?, cd_started = ?, cd_caring = ?, cd_gift = ?, dismiss_time = ?, mood = ? WHERE player_id = ? AND pet_id = ?");
            ...
            stmt.setLong(6, pet.getCdCaring());
            stmt.setLong(7, pet.getCdGift());
            stmt.setLong(8, pet.getDismissTime());
            stmt.setInt(9, pet.getMood());
            stmt.setInt(10, player.getObjectId());
            stmt.setInt(11, pet.getPetId());
            stmt.execute();
            stmt.close();
        } catch (Exception e) {
	...
	@Override
    public void setMood(Player player, int petId, int mood) {
        Connection con = null;
        try {
            con = DatabaseFactory.getConnection();
            PreparedStatement stmt = con.prepareStatement("UPDATE player_pets SET mood = ? WHERE player_id = ? AND pet_id = ?");
            stmt.setInt(1, mood);
            stmt.setInt(2, player.getObjectId());
            stmt.setInt(3, petId);
            stmt.execute();
            ...
            log.error("Error updating pet mood #" + petId , e);
        } finally {
            DatabaseFactory.close(con);
        }
    }
